...
public class MyGame implements Screen {
	...
	public void render(float delta) {
		...
		if(Gdx.input.isTouched(3)) {
			for(int i=0; i<5; i++) account.nextTime();
		}
		...
		// Next Time of Game
		if(Home.nowSleep || TimeUtils.nanoTime()-clockGame >= 300000000) {
			account.nextTime();
			clockGame = TimeUtils.nanoTime();
		}
		
		// when Want to Save In Windows
		...
		if(Home.nowSleep) {
			...
			if(Gdx.input.isTouched(1) && !Gdx.input.isTouched(2)) {
				if(Gdx.input.justTouched()) {
					originalDistance = (float) Math.sqrt(Math.pow((MyMain.mouseX[0] - MyMain.mouseX[1]), 2) + Math.pow((MyMain.mouseY[0] - MyMain.mouseY[1]), 2));
				} else {
					currentDistance = (float)Math.sqrt(Math.pow((MyMain.mouseX[0]-MyMain.mouseX[1]), 2)+Math.pow((MyMain.mouseY[0]-MyMain.mouseY[1]), 2));
					if(MyGame.zoom+((currentDistance/originalDistance)-1)/2 > 1 && MyGame.zoom+((currentDistance/originalDistance)-1)/2 < 2) {
						MyGame.zoom += ((currentDistance / originalDistance) - 1) / 2;
						MyGame.moveX -= 500 * ((currentDistance / originalDistance) - 1) / MyGame.zoom;
						MyGame.moveY -= 250 * ((currentDistance / originalDistance) - 1) / MyGame.zoom;
						originalDistance = currentDistance;
						if (MyGame.moveX < MyGame.zoom * (-600) + 10) {
							MyGame.moveX = MyGame.zoom * (-600) + 10;
						} else if (MyGame.moveX > 0) MyGame.moveX = 0;
						if (MyGame.moveY < MyGame.zoom * (-350) + 10) {
							MyGame.moveY = MyGame.zoom * (-350) + 10;
						} else if (MyGame.moveY > 0) MyGame.moveY = 0;
					}
				}
			}
			else if(Gdx.input.isTouched() && !BuySeed.nowBuySeed) {
				if(Button.nowSelectTool == 0) {
					if(Gdx.input.justTouched()) { button.mouseX = MyMain.mouseX[0];button.mouseY = MyMain.mouseY[0]; }
					else {
						moveX += (MyMain.mouseX[0]-button.mouseX)/zoom;
						moveY += (MyMain.mouseY[0]-button.mouseY)/zoom;
						...
						button.mouseX = MyMain.mouseX[0];button.mouseY = MyMain.mouseY[0];
					}
				}
				else {
			...
			if(planeRect.overlaps(new Rectangle(MyMain.mouseX[0], MyMain.mouseY[0], 0.5f, 0.5f))) {
				nowTouchPlane = true;
