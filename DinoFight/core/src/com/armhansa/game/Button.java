...
//		pm[4] = new Pixmap(Gdx.files.internal("cursor-2.png"));
	...
	public void checkAction() {
		for(byte i=0;i<5;i++) {
			if(btn[i].overlaps(new Circle(MyMain.mouseX[0], MyMain.mouseY[0], 0.5f))) {
				touchedBtn = i;
				...
				if(Gdx.input.isTouched() && Gdx.input.justTouched() && i != nowSelectTool) {
					...
					if(i==0) { mouseX = MyMain.mouseX[0];mouseY = MyMain.mouseY[0]; }
				}
			}
			else justTouchedBtn[i] = true;
			if((nowSelectTool == 0 || nowSelectTool == 1) && i < 3) {
				if(seedRect[i].overlaps(new Rectangle(MyMain.mouseX[0], MyMain.mouseY[0], 0.5f, 0.5f))) {
					touchedSeed = i;
